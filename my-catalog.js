...
const updateProductDescription = () => {
    $('.with-line-attributes').each((product) => {
        if ($('.card-bottom.with-line-attributes')[product]) {
            const bottomCard = $('.card-bottom.with-line-attributes')[product];
            const hasWarning = $(bottomCard).find('.message.warning').length;
        }

        if ($('.information.with-line-attributes')[product]) {
            const thisItemNumber = $('.information.with-line-attributes')[product].children[2].children[1].children[1].innerText;
            const thisProduct = $('.information.with-line-attributes')[product].children[0].children[0]
            const thisDescription = $('.information.with-line-attributes')[product].children[2].children[0];

            if ($(thisProduct).length) {
                const title = $(thisProduct)[0];
                if ($(title).text() != $(thisDescription).text() && $(thisDescription).text() != "") {

                    const titleShort = $(title).text().slice();
                    let desc = $(thisDescription).text().slice();

                    desc = `<div id="title">${$(title).text()}</div><div class="description">${desc}</div>`;



                    if (!$(thisProduct).hasClass("changed")) {

                        $(thisProduct).html(desc);
                        $(thisProduct).addClass("changed");
                    }
